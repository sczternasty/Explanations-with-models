...
def unfold_tbox(concept, tbox, iter_count=0, max_iter=1000):
    ...
    for axiom in tbox:
        if axiom.getClass().getSimpleName() == "GeneralConceptInclusion":
            if concept == formatter.format(
                ...
                rhs = unfold(axiom.rhs(), tbox, iter_count + 1, max_iter)
...
def unfold(concept, tbox, iter_count=0, max_iter=100):
    ...
    if conceptType == "ConceptName":
        ...
        if conceptType == "ConceptConjunction":
            ...
            conjunct1 = unfold(conjuncts[0], tbox, iter_count + 1, max_iter)
            conjunct2 = unfold(conjuncts[1], tbox, iter_count + 1, max_iter)
            return elFactory.getConjunction(conjunct1, conjunct2)
        if conceptType == "ExistentialRoleRestriction":
            role = concept.role()
            filler = unfold_tbox(formatter.format(concept.filler()), tbox, iter_count + 1, max_iter)
